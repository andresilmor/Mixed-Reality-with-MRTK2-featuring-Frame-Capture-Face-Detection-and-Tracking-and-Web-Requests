...
public class InterfaceAnimManager : MonoBehaviour {
    ...
    private InterfaceAnimElementsList interfaceAnimElementsList;
    ...
    void Start() {
        ...
        interfaceAnimElementsList = gameObject.GetComponent<InterfaceAnimElementsList>();   

        Debug.Log(elementsList.Count);
    }
                        ...
                        foreach (InterfaceAnmElement _element in elementsList) {
                            ...
                            if (_element.animator && _element.gameObjectRef.activeSelf) { // 1.6.0 check if active, if not, unity would trigger a 'Animator is not playing a Playable' error
                                for (int layerIndex = 0; layerIndex < _element.animator.layerCount; layerIndex++) {
                                    if (_element.animator.gameObject.activeSelf)
                            ...
                            if (!useDelays || timer > _element.timeAppear * appearDelay_SpeedMultiplier) {
                                ...
                                if (_element.currentState == CSFHIAnimableState.disappeared) {
                                    ...
                                    if (_element.animator && _element.currentState == CSFHIAnimableState.appearing) {
                                        ...
                                        if (_element.animator.GetCurrentAnimatorStateInfo(0).IsName(animName) && _element.animator.gameObject.activeSelf && _element.animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1 && !_element.animator.IsInTransition(0)) {
                                            if (_element.animator.gameObject.activeSelf)
                                                _element.currentState = CSFHIAnimableState.appeared;
                                        }
                                    }
    ...
    public void UpdateAnimClips() {
        ...
        */
        ...
        if (interfaceAnimElementsList is null)
            this.interfaceAnimElementsList = this.gameObject.GetComponent<InterfaceAnimElementsList>();
        InterfaceAnmElement[] gameObjects = this.interfaceAnimElementsList.gameObjects;

        foreach (InterfaceAnmElement child in gameObjects)
        {
          
                addElement = true;
                //foreach loop won't work since we remove elements, so we go trough a inverted for loop
                for (int a = gameObjects.Length - 1; a >= 0; a--)
                {
                    //Debug.Log(_target.gameObject.transform.name);
                    //Debug.Log(_target.elementsList[a].gameObjectRef.transform.parent.name);
                        if (gameObjects[a].gameObjectRef == child.gameObject)
                        {
                            addElement = false;
                            gameObjects[a].sortID = newSortID;
                            break;
                        }
                    }
                if (addElement)
