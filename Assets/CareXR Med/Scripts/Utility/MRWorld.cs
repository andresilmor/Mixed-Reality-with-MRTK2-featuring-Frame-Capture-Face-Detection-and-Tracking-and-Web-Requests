...
public static class MRWorld {
    ...
    private static SpatialCoordinateSystem RetrieveWorldOriginFromPointer(SpatialCoordinateSystem worldOriginPtr)
        ...
        public static Windows.Foundation.Point GetBoundingBoxTarget(Rect2d rect, Vector4 cameraForward)
        {
            ...
            return new Windows.Foundation.Point(point.x, Controller.CameraMain.pixelHeight - point.y); //846 = Frame Height, static now but we can turn dynamic and get from Intrinsics
        }
    ...
    public static Vector2 GetUnprojectionOffset(float posY) {
        ...
        /*
        if (posY > Controller.CameraMain.pixelHeight / 2) // Got by trial and error / Negative = UP / Positive = Down
        {
    ...
    public static Vector3 GetPosition(Vector3 cameraPosition, Vector3 layForward, int layer) {
        ...
        {
            Controller.Instance.DetectionDistanceLimit.position = cameraPosition;
        ...
        //UnityEngine.Object.Instantiate(XRDebug.GetCubeForTest(), hit.point, Quaternion.identity);
    ...
    public static void GetFaceWorldPosition(out Vector3 worldPosition, BoxRect boxRect, CameraFrame cameraFrame) {
        ...
        if (testWorldPosition.y > Controller.CameraMain.pixelHeight / 2) // Got by trial and error / Negative = UP / Positive = Down
        {
