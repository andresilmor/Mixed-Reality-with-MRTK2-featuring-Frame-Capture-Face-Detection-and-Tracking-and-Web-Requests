...
public static class TrackerManager {
    ...
    public static TrackerHandler CreateTracker(BoxRect boxRect, Mat frame, Vector3 worldPosition, TrackerType trackerType) {
        ...
        */
        ...
        newVisualTracker.transform.LookAt(Controller.CameraMain.transform);
        ...
        Debug.Log("PositionZ");
        Controller.Strech(newVisualTracker, layForwardBottom, layForwardTop, true);
        Debug.Log("Strech");
    ...
    public static IEnumerator UpdateTrackers() {
        ...
        while (true) {
...
#if ENABLE_WINMD_SUPPORT
                    rect = tracker.Value.UpdateTracker(tracker.Value.TrackerSettings.boundingBox, MediaCaptureManager.GenerateCVMat(Controller.MediaCaptureManager.LastFrame.mediaFrameReference));
#endif
                            if (rect == null) {
            ...
            if (_liveTrackers.Count > 0)
                Controller.Instance.timeToStop += 1;

            yield return new WaitForEndOfFrame();
            if (_liveTrackers.Count > 0)
                Debug.Log("Round: " + Controller.Instance.timeToStop);
