...
/*
        if (Controller.MediaCaptureManager == null) {
            Debug.Log("wtf");
            Controller.MediaCaptureManager = await MediaCaptureManager.CreateAsync();
        } else if (!MediaCaptureManager.IsCapturing) {
        Debug.Log("START CAPTURE");
            await Controller.MediaCaptureManager.StartCapture();
        }*/
#endif
        ...
        switch (detectionMode) {
            case DetectionMode.OneShot:
                ...
                //Controller.MediaCaptureManager.FrameArrived += OneShotDetection;
                break;
...
/*
    ...
    private async static void OneShotDetection(object sender, FrameArrivedEventArgs e) {
        ...
        if (_wsConnected) {
            ...
            if (_hasFinishedOneShot)
                wsTemp = APIManager.GetWebSocket(APIManager.QRDecode);
            else 
                return;
            ...
            try {
                using (var videoFrame = e.Frame.MediaFrameReference.VideoMediaFrame.GetVideoFrame()) {
                    if (videoFrame != null && videoFrame.SoftwareBitmap != null) {
                        ...
                        wsTemp.Send(Parser.ProtoSerialize<ProtoImage>(request));
                        Debug.Log("Sended");
        ...
        Debug.Log("HERE .....");
        WebSocket ws = APIManager.CreateWebSocketConnection( APIManager.QRDecode, (WebSocket ws, string message) => {
            Debug.Log("Action .....");
            try {
    ...
    private static void StopOneShotDetection(List<QRCodeDecodeReply.Detection> qrCodes) {
        //Controller.MediaCaptureManager.FrameArrived -= OneShotDetection;
        APIManager.GetWebSocket(APIManager.QRDecode).Close();
        ...
        private static void ProcessDetections(object sender, FrameArrivedEventArgs e) {
            ...
            try {
                ...
                if (result) {
                    ...
                    Controller.Instance.StopCoroutine(_stopDetectitionCoroutine);
                    ...
                    if (!InPassiveMode && _timerActions.Count <= 0) {
                        Controller.MediaCaptureManager.FrameArrived -= ProcessDetections;
                        _detecting = false;
