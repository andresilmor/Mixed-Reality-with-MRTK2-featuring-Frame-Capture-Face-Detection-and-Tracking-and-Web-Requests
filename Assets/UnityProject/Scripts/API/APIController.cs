...
;
        ...
        Vector2 unprojection = CameraIntrinsic.UnprojectAtUnitDepth(target, frameHandler.LastFrame.intrinsic).ToUnity();
        unprojection = CameraIntrinsic.UnprojectAtUnitDepth(new Windows.Foundation.Point(b.centerX, 936 - b.centerY), frameHandler.LastFrame.intrinsic).ToUnity();
                ...
                using (var videoFrame = lastFrame.mediaFrameReference.VideoMediaFrame.GetVideoFrame())
                {
                    if (videoFrame != null && videoFrame.SoftwareBitmap != null)
                    {
                       ...
                       Instantiate(sphereForTest, Camera.main.transform.position, Quaternion.identity);
                        ...
                        debugText.text = debugText.text + "\n vIDEO fORMAT WIDTH: " + lastFrame.mediaFrameReference.VideoMediaFrame.VideoFormat.Width;
                        debugText.text = debugText.text + "\n vIDEO fORMAT HEIGHT: " + lastFrame.mediaFrameReference.VideoMediaFrame.VideoFormat.Height;
